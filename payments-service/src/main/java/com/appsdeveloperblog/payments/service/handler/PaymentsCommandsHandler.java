...
public class PaymentsCommandsHandler {
    ...
    public void handleCommand(@Payload ProcessPaymentCommand command) {
        ...
        try {
            Payment payment = new Payment(command.getOrderdId(),
                    command.getProductId(),
                    command.getProductPrice(),
                    command.getProductQuantity());
            Payment proccessdPayment = paymentService.process(payment);
            PaymentProcessedEvent paymentProcessedEvent = new PaymentProcessedEvent(proccessdPayment.getOrderId(),proccessdPayment.getId());
            kafkaTemplate.send(paymentEventsTopicName, paymentProcessedEvent);
        }catch (CreditCardProcessorUnavailableException e){
            logger.error(e.getLocalizedMessage(),e);
            PaymentFailedEvent paymentFailedEvent = new PaymentFailedEvent(command.getOrderdId(),
                    command.getProductId(),
                    command.getProductQuantity());
